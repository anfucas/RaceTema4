...
```{r}
cran <- as_tibble(mydf) #Transformamos en tibble 
rm(mydf) #Borramos los datos de mydf
...
```{r}
cran%>%select(c("ip_id","package","country")) #Elegimos las variables ip_id, package y country
```
5. Elige todas las variables entre `r_arch y country` 
```{r}
cran%>%select(c("r_arch":"country")) #Elegimos las variables entre r_arch y country
```
...
```{r}
cran %>% select(-c(date:size)) #Seleccionamos todas las columnas de cran menos las que van entre date y size
```
...
```{r}
cran %>% filter(package == "plotly", r_version == "3.5.1") #Elegimos los paquetes plotly con r version 3.5.1
```
6. Elige aquellas descargas realizadas desde España (ES), con versiones de R superiores a la 3.4.0
```{r}
cran%>%filter(country=="ES", r_version > "3.4.0") #Elegimos las descargas en ES con r version 3.4.0
```

7. Elige aquellas descargas realizadas desde España (ES), o Portugal (PT).
```{r}
cran%>%filter(country %in% c("ES", "PT")) #Elegir descargas desde ES o PT
```
...
```{r}
cran2 <- cran%>%select(size:ip_id) #Almacenamos las columnas
cran2
```
11. Ordena `cran2` en orden ascendente según la variable `ip_id`.
```{r}
cran2 <- cran2%>%arrange(ip_id) #Orden de ip_id ascendente
cran2
```
12. Ordena `cran2` en orden descendente según la variable `ip_id`.
```{r}
cran2 <- cran2%>%arrange(desc(ip_id)) #Orden de ip_id descendente
cran2
```
12. Ordena `cran2` según los valores de `package e ip_id`.
```{r}
cran2 <- cran%>%arrange(package, ip_id) #Ordenamos los valores
```
...
```{r}
by_package %>% summarise(package_size = mean(size, na.rm = TRUE))
```
...
```{r}
pack_sum <- by_package%>%summarise(count=n(), unique=n_distinct(ip_id), countries=n_distinct(country), avg_bytes = stats::median(size, na.rm = TRUE))
pack_sum
```
